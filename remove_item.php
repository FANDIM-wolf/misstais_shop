...
if(!empty($result)){
	...
	echo $quantity;
	if($quantity == 0){
		$sql_delete ="DELETE FROM `orders` WHERE id_of_good =:id_of_good AND name_of_user = :name_of_user";
		$statement_delete =  $pdo->prepare($sql_delete);

		$statement_delete->execute([':name_of_user' => $current_user ,  'id_of_good' => $add_item]);
